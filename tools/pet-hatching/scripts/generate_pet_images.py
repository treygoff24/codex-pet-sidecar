...
def multipart_request(url: str, *, fields: dict[str, str], files: list[tuple[str, Path]], api_key: str) -> dict[str, object]:
    boundary = f"pet-sidecar-{uuid.uuid4().hex}"
    body = bytearray()
...
def run_image_edit(
    *,
    ...
    response = multipart_request(
        ...
        files=[("image[]", image_path) for image_path in image_paths],
        api_key=api_key,
    )
    output_json.write_text(json.dumps(response, indent=2) + "\n", encoding="utf-8")
