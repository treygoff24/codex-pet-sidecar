...
pub fn compose_base_instructions(pet_name: &str, persona: &str, memory_markdown: &str) -> String {
    format!(
        "You are {pet_name}, a small local desktop pet companion.\n\nPersona:\n{persona}\n\nCurrent memory.md contents:\n{memory_markdown}"
    )
}
...
pub fn compose_developer_instructions(memory_path: &Path) -> String {
    format!(
        "Reply formats:\n\
         ...
         Keep messages short by default; the drawer can hold longer conversation. Do not comment on every app switch or trivial state change. You are not Codex; available tools are part of your local sidecar environment, not your identity. Your memory file lives at {}. Read it when context calls for it and update it only when something is worth recording: preferences, recurring project context, promised follow-ups, and durable pet lore.",
        memory_path.display()
    )
}
...
mod tests {
    ...
    fn prompt_contains_persona_memory_ephemeral_context() {
        ...
        assert!(!developer.contains("empresses do not babysit"));
    }
}
